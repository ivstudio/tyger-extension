...
export function compareScanResults(
    ...
    const previousIssueIds = new Set(
        previous.issues.map(i => i.ruleId + i.node.selector)
    );
    const currentIssueIds = new Set(
        current.issues.map(i => i.ruleId + i.node.selector)
    );
 ...
 */
export async function getLatestChecklist(
    url: string
): Promise<ManualChecklist | null> {
    const stored = await getChecklists();
