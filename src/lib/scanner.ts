 ...
 */
function processAxeResults(result: Result, type: 'violation' | 'incomplete'): Issue[] {
  const issues: Issue[] = [];

  for (const node of result.nodes) {
    const selector = Array.isArray(node.target[0]) ? node.target[0].join(' ') : node.target[0];
    ...
    const issue: Issue = {
      ...
      tags: result.tags,
      timestamp: Date.now(),
      notes: type === 'incomplete' ? 'This issue requires manual verification' : undefined
    };
...
function determineWCAGLevel(tags: string[]): 'A' | 'AA' | 'AAA' {
  ...
  if (tags.some(tag => tag.includes('wcag2aa') || tag.includes('wcag21aa') || tag.includes('wcag22aa'))) {
    return 'AA';
  }
  return 'A';
}
...
function generateRecommendations(
  ...
  const devRec: Recommendation = {
    ...
    priority: result.impact === 'critical' || result.impact === 'serious' ? 'high' : 'medium'
  };
...
function getDesignRecommendation(ruleId: string): string {
  const recommendations: Record<string, string> = {
    'color-contrast': 'Ensure text has sufficient contrast ratio (4.5:1 for normal text, 3:1 for large text). Use a color contrast checker.',
    'link-in-text-block': 'Make links visually distinct from surrounding text (underline, different color with 3:1 contrast).',
    'focus-order-semantics': 'Design visual layout to match logical tab order.',
    'target-size': 'Ensure interactive elements are at least 44x44px (or 24x24px with sufficient spacing).',
    'label-content-name-mismatch': 'Ensure visible label text matches or is contained in accessible name.'
  };
